...
def font():
    ...
    fontbox = tkinter.Listbox(font, width=20, height=9, listvariable=typeface, selectmode="browse")  
    ...
    cancel = tkinter.Button(font, text="Canael",
        width=8, height=1, 
        fg='white', bg='dim gray',
        font=('Arial', 8, 'bold'))
...
class Font(object):
    def __init__(self, initface="Arial", initsize=10, initthick='bold'):
        self.initface = initface
        self.initsize = initsize
        self.initthick = initthick

    def fontFace(self, face='none', size='none', thick='bold'):
        face = self.initface
        size = self.initsize
...
def findDialog():
    ...
    background = tkinter.Label(findwindow, width=10, height=5, bg='dim gray')
    background.place(x = 0, y = 0,)

    findlabel = tkinter.Label(findwindow, text="Find", fg='light gray', bg='dim gray', font=('Arial', 12))
    findlabel.place(x = 0, y = 0)

    findbox = tkinter.Entry(findwindow, width=25, textvariable=character, fg='white', bg='dim gray', font=("Arial", 16))
    findbox.grid(row=0, column=1)

    findbox.focus_set()

    replacelabel = tkinter.Label(findwindow, text="Replace", fg='light gray', bg='dim gray', font=('Arial', 12))
    replacelabel.grid(row=1, column=0, stick="e")

    replacebox = tkinter.Entry(findwindow, width=25, fg='white', bg='dim gray', font=("Arial", 16))
    replacebox.grid(row=1, column=1)

    caeswrite = tkinter.Checkbutton(findwindow, text="", bg='dim gray', variable=integer)
...
def find(stringvar, intvar, text, findwindow, findbox):
    ...
    if stringvar:
        position = "1.0"
        while True:
            position = text.search(stringvar, position, nocase=intvar, stopindex='end')
            if not position: 
                break
...
class MainMenuBar(object):
    ...
    def fileMenu(self):
        ...
        fileoption.add_command(label=file[1], accelerator="Ctrl+N", command=FileOperation.newFile, font=(Font.fontFace()))
        fileoption.add_command(label=file[2], accelerator="Ctrl+Z", command=AskDialog.openDialog, font=(Font.fontFace()))
        fileoption.add_command(label=file[3], accelerator="Ctrl+S", command=FileOperation.saveFile, font=(Font.fontFace()))
        fileoption.add_command(label = file[4], command = FileOperation.saveAsFile, font = (Font.fontFace())) 
        ...
        fileoption.add_cascade(label=file[5], 
            font=(Font.fontFace()), menu=saveencode)

        fileoption.add_command(label=file[6], 
            command=FileOperation.closeFile, font=(Font.fontFace()))
        fileoption.add_separator()
        fileoption.add_command(label=file[7], 
            command=message.MessageBox.printMessage, font=(Font.fontFace()))
        ...
        fileoption.add_command(label = file[9], 
            command = message.MessageBox.exitMessage, font = (Font.fontFace()))

        self.menubar.add_cascade(label = file[0], menu = fileoption,  font = (Font.fontFace()))
    ...
    def editMenu(self):
        ...
        editoption.add_command(label=edit[4], accelerator="Ctrl+C", 
            command=Edit.copy, font=(Font.fontFace()))

        editoption.add_command(label=edit[5], accelerator="Ctrl+V", 
            command=Edit.paste, font=(Font.fontFace()))
        editoption.add_command(label=edit[6], accelerator="Del", font=(Font.fontFace()))
        editoption.add_separator()
        editoption.add_command(label=edit[7], accelerator="Ctrl+F",
            command=findDialog, font=(Font.fontFace()))

        editoption.add_command(label=edit[8], accelerator="F3", font=(Font.fontFace()))
        ...
        editoption.add_command(label=edit[10], accelerator="Ctrl+A", 
            command=Edit.selectAll, font=(Font.fontFace()))

        editoption.add_command(label=edit[11], accelerator="F5", 
            command=localTime, font=(Font.fontFace()))

        self.menubar.add_cascade(label=edit[0], menu=editoption)
    
    def formatMenu(self):
        formatoption = tkinter.Menu(self.menubar, tearoff=0)
        formatoption.add_checkbutton(label=formats[1], command=line,font=(Font.fontFace()))
        formatoption.add_separator()
        formatoption.add_command(label=formats[2], command=font, font=(Font.fontFace()))
        self.menubar.add_cascade(label=formats[0], menu=formatoption)

    def viewMenu(self):
        viewoption = tkinter.Menu(self.menubar, tearoff=0)
        viewoption.add_checkbutton(label=view[1], command=copyRight, font=(Font.fontFace()))
    ...
    def helpMenu(self):
        ...
        background.add_radiobutton(label=back[0], command=BackGround.lightGround, font=(Font.fontFace()))
        background.add_radiobutton(label=back[1], command=BackGround.darkGround, font=(Font.fontFace()))
        background.add_separator()
        background.add_radiobutton(label=back[2], command=AskDialog.colorDialog, font=(Font.fontFace()))
        helpoption.add_cascade(label=helpping[1], menu=background, font=(Font.fontFace()))

        helpoption.add_separator()
        helpoption.add_command(label=helpping[2], command=message.MessageBox.aboutIbook, font=(Font.fontFace()))
...
class MouseRightClick(object):
    ...
    def rightClick(self, event):
        ...
        rclick_options.add_command(label=edit[1], command=Edit.undo, font=(Font.fontFace()))
        rclick_options.add_command(label=edit[2], command=Edit.redo, font=(Font.fontFace()))
        rclick_options.add_separator()
        rclick_options.add_command(label=edit[3], command=Edit.cut, font=(Font.fontFace()))
        rclick_options.add_command(label=edit[4], command=Edit.copy, font=(Font.fontFace()))
        rclick_options.add_command(label=edit[5], command=Edit.paste, font=(Font.fontFace()))
        rclick_options.add_command(label=edit[6], command=Edit.undo, font=(Font.fontFace()))
        rclick_options.add_separator()
        rclick_options.add_command(label=edit[10], command=Edit.selectAll, font=(Font.fontFace()))
        rclick_options.add_separator()

        linked_menu = tkinter.Menu(rclick_options, tearoff=0)
        linked_menu.add_radiobutton(label="chinese simplified", font=(Font.fontFace()))
        linked_menu.add_radiobutton(label="chinese Traditional", font=(Font.fontFace()))
        linked_menu.add_separator()
        linked_menu.add_radiobutton(label="Korean(EUC-KR)", font=(Font.fontFace()))
        linked_menu.add_separator()
        linked_menu.add_radiobutton(label="Japanese(CP932)", font=(Font.fontFace()))
        linked_menu.add_radiobutton(label="Japanese(Shift_JIS)", font=(Font.fontFace()))
        linked_menu.add_radiobutton(label="Japanese(EUC-JP)", font=(Font.fontFace()))
        linked_menu.add_separator()
        linked_menu.add_radiobutton(label="UTF-8", font=(Font.fontFace()))

        rclick_options.add_cascade(label="Set Encoding...   ", font=(Font.fontFace()), menu=linked_menu)       
...
class AskDialog(object):
    def __init__(self, askobject=tkinter.filedialog, chooseobject=tkinter.colorchooser):
