...
class Font(object):
	def __init__(self, initface = "微软雅黑", 
...
def find(stringvar, intvar, text, findwindow, findbox):
	...
	if stringvar:
		...
		findwindow.title("搜索到" + str(count) + "条结果")
...
class MouseRightClick(object):
	...
	def rightClick(self, event):
		...
		rclick_options.add_cascade(label = "设置字符编码...   ", 
								   font = (Font.fontFace()), 
								   menu = linked_menu)       
...
class AskDialog(object):
	...
	def openDialog(self):
		global filename
		filename = self.askobject.askopenfilename(title = 'ibook 打开文件',
	...
	# def saveDialog(self):
	#     savename = self.askobject.asksaveasfilename(title = 'ibook 文件保存为', 
	...
	def saveAsDialog(self):
		global filename
		saveasname = self.askobject.asksaveasfilename(title = 'ibook 文件保存为', 
			initialdir = r'C:\ ibook',                                                       
			filetypes = [('All Files', '*'), ('文本文档', '*.txt'), 
						 ('Python', '*.py *.pyw'),('Java', '*.java'), 
						 ('C', '*.c *.h')], 
	...
	def colorDialog(self):
		color = self.chooseobject.askcolor(title = 'ibook 背景颜色')
		_color = color[0]
...
class FileOperation(object):
	def newFile(self):
		new = tkinter.messagebox.askokcancel("ibook 保存文件", "保存文件")
		if new == True:
			filename = None
